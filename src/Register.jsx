...
export default function Register() {
  ...
  const onsubmit = async (data) => {
    ...
    let datas = axiosClient.post("/signUp", newData).then((res) => {
      navigate("/");
    });
  };
  return (
      ...
      <div className="w-[100%] h-[70vh] p-4 bg-gray-400 flex flex-col justify-center items-center lg:h-full">
        ...
        <form onSubmit={handleSubmit(onsubmit)} className="w-[50%]">
          <div className="relative">
            <PersonIcon className="absolute top-[27px] left-3" />
          ...
          <div>
            <button
              type="submit"
              className="bg-bluishGreen-100 px-4 py-2 rounded-md mt-2 text-white text-lg hover:text-bluishGreen-100 hover:bg-white"
            >
              Register
            </button>
          </div>
